...
# PROVENANCE: the image has no git or .git. nexclamp.provenance.git_state() falls back to
...
# 3) Project source, editable install (nexclamp.provenance.REPO_ROOT resolves to the source
...
RUN python -c "from nexclamp.simulators.jneuroml import JNeuroML; s = JNeuroML(); assert s.available(), 'Java or jNeuroML jar missing'; print(s.java, s.jar); print(s.version_string())"
