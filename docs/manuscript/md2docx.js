// Convert the manuscript Markdown into a shareable Word document.
const fs = require("fs");
...
const src = fs.readFileSync(path.join(REPO, "docs/manuscript/MANUSCRIPT.md"), "utf8").replace(/\r/g, "");
const out = process.argv[2] || path.join(REPO, "docs/manuscript/MANUSCRIPT_v0.1.docx");
const FIGS = ["h1_per_model_primary", "h2_primary_difference", "h3_validation_levels",
              "h4_random_baselines", "h5_attrition"].map(f => path.join(REPO, "results/figures/heldout-v1", f + ".png"));
const FONT = "Times New Roman", SIZE = 22, CONTENT = 9360;
...
while (i < lines.length) {
  ...
  if (/^## /.test(l)) { flush(); inFigures = /Figure legends/.test(l);
