...
from neuraxis.models import Workspace, load_models, materialize, snapshot_dir
from neuraxis.provenance import (REPO_ROOT, git_state, sha256_bytes, sha256_file, sha256_json, tree_manifest,
                                 utc_now, write_immutable_text)
from neuraxis.schemas import ModelRecord, RunStatus, VariantKind, VariantRecord, dumps, to_jsonable
...
MANDATORY_EXCLUDES: tuple[str, ...] = (
    ...
    "configs/agent_policy.yaml", "configs/FROZEN.lock", "src/neuraxis/experiments/agent.py", "AI_USE_LOG.md",
    "**/PROVENANCE.json",
)
...
def load_policy(path: Path = POLICY_PATH) -> AgentPolicy:
    ...
    missing = [p for p in MANDATORY_EXCLUDES if p not in (d["export"].get("mandatory_exclude") or ())]
    if missing:
...
def load_operator_registry(registry: str) -> Mapping[str, Any]:
    """Import ``neuraxis.mutations`` / ``neuraxis.transforms`` lazily and return its ``REGISTRY``.
...
def prepare_trial(task_id: str, dest: Path, *, private_dir: Path | None = None, private_root: Path = PRIVATE_ROOT,
    ...
    state = source_state(repo, [task.task_dir, common, policy.path, repo / "src" / "neuraxis", snapshot_dir(model)])
...
class DefaultEvaluators:
    ...
    def canonical_passes(self, ctx: ScoringContext) -> LayerOutcome:
        ...
        try:
            ...
            from neuraxis.validation.fingerprint import Fingerprint, compare
    ...
    def hidden_battery_passes(self, ctx: ScoringContext) -> LayerOutcome:
        ...
        if not b.get("applicable"):
            return LayerOutcome(name, LayerStatus.NOT_APPLICABLE)
        try:
            from neuraxis.protocols.definitions import CANONICAL_ID, templates_from_config
            from neuraxis.schemas import ExecConfig
            from neuraxis.validation.canonical import canonical_protocol
            from neuraxis.validation.fingerprint import RHEOBASE_ID, build_fingerprint, compare
        except ImportError as exc:
...
def score_trial(task_id: str, trial_dir: Path, frozen_config: FrozenConfig | Path | str, *,
    ...
    if not frozen_config.provisional:
        repo = Path(repo_root)
        frozen_config.check_evaluator_state(source_state(repo, [repo / "src" / "neuraxis", repo / "agent_study",
                                                                repo / "configs"]))
