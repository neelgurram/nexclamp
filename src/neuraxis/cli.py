...
Every command is a thin wrapper around ``neuraxis.experiments``. Commands never touch
...
def cmd_select(a) -> int:
    from neuraxis.experiments.discovery import select_protocols
...
def cmd_heldout(a) -> int:
    from neuraxis.experiments.heldout import evaluate_heldout
...
def cmd_analyze(a) -> int:
    from neuraxis.experiments.analyze import analyze_campaign
...
def cmd_reproduce(a) -> int:
    from neuraxis.experiments.analyze import reproduce_paper
...
def cmd_curate(a) -> int:
    from neuraxis.orchestration.curation import run_curation

    print(run_curation(a.campaign, [Path(s) for s in a.sources], Path(a.out), budget_s=a.budget_s,
                       models=a.models or None, workers=a.workers))
    return 0


def cmd_smoke_test(a) -> int:
    from neuraxis.orchestration.smoke import FIXTURES, run_smoke

    report = run_smoke(Path(a.out), a.models or FIXTURES, run_tests=not a.skip_tests)
    print(json.dumps({k: v["ok"] for k, v in report["steps"].items()}, indent=2))
    print("PASSED" if report["passed"] else "FAILED")
    return 0 if report["passed"] else 1


def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(prog="neuraxis", description=__doc__)
    ...
    p = sub.add_parser("curate-models", help="apply the model inclusion/exclusion criteria (reference simulations only)")
    p.add_argument("--campaign", default="curation-v1")
    ...
    p = sub.add_parser("smoke-test", help="infrastructure smoke test (the execution plan's nine checks)")
    p.add_argument("--out", required=True)
    p.add_argument("--skip-tests", action="store_true")
    _models_arg(p)
    p.set_defaults(fn=cmd_smoke_test)
    return ap
