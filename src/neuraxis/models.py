...
from neuraxis.provenance import REPO_ROOT, tree_sha256
from neuraxis.schemas import ModelRecord
...
def snapshot_dir(model: ModelRecord) -> Path:
    """The stored snapshot holding this model's cell file (``models/raw`` first, then ``models/candidates``)."""
    for root in (RAW_MODELS, CANDIDATE_MODELS):
        if (root / model.snapshot / model.cell_file).is_file():
            return root / model.snapshot
    for root in (RAW_MODELS, CANDIDATE_MODELS):
        if (root / model.snapshot).is_dir():
            return root / model.snapshot
