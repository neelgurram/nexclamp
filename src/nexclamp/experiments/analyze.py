...
from nexclamp.schemas import MutantClass
...
def reclassify(campaign: str, multipliers: list[float]) -> pd.DataFrame:
    ...
    from nexclamp.validation.fingerprint import classify, compare, load_fingerprint
...
def _load_trace(campaign: str, run_id: str, name: str):
    from nexclamp.features import trace_metrics
...
def analyze_campaign(campaign: str, n_boot: int = 2000, n_perm: int = 2000, draws: int | None = None) -> Path:
    from nexclamp.analysis import bootstrap, figures, metrics
