...
checks guarantee that: every operator records an :class:`~nexclamp.schemas.Edit` for each
...
from nexclamp.provenance import sha256_json, tree_manifest, utc_now
...
from nexclamp.units import UNITS
...
def quantity_si(text: str) -> float:
    ...
    if unit not in UNITS:
        raise MutationError(f"unit {unit!r} in {text!r} is not supported by nexclamp.units")
