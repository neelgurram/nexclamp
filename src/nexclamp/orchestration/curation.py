...
from nexclamp.models import MANIFEST_COLUMNS, materialize, normalise_fields
from nexclamp.protocols.definitions import CANONICAL_ID, batched, templates_from_config
from nexclamp.provenance import git_state, utc_now
from nexclamp.schemas import ModelRecord, RunStatus
...
def curate_one(ctx: Any, model: ModelRecord, *, budget_s: float, seed: int,
    ...
    from nexclamp.validation.convergence import EXCLUDED_DEFINEDNESS, EXCLUDED_REGIME, calibrate
    from nexclamp.validation.fingerprint import build_fingerprint
...
def run_curation(campaign: str, sources: Sequence[Path], out_csv: Path, *, budget_s: float = 4200.0,
    ...
    from nexclamp.validation.execution import run_parallel
...
def write_report(rows: Sequence[dict], path: Path, campaign: str, stamp: str, budget_s: float) -> None:
    ...
    lines = ["# Model curation report", "",
             f"*Generated by `nexclamp curate-models` (campaign `{campaign}`, {stamp}). Only reference models were "
             "simulated; no mutant was run, so no detection outcome was seen. Criteria are mechanical and "
             "decided before any split (`src/nexclamp/orchestration/curation.py`).*", "",
             "## In plain English", "",
