...
from nexclamp.features.regimes import firing_regime
from nexclamp.schemas import ConcreteProtocol, Trace
...
def _data(cfg: Any) -> Mapping[str, Any]:
    data = getattr(cfg, "data", cfg)          # accepts nexclamp.config.LoadedConfig or a plain dict
...
def extract(trace: Trace, protocol: ConcreteProtocol, cfg: Any, warnings: list[str] | None = None) -> FeatureTable:
    ...
    :func:`~nexclamp.features.trace_metrics.unpack_traces` or
    :func:`~nexclamp.features.trace_metrics.stored_representation`) so the numbers
