...
def env_var(suffix: str, default: str | None = None) -> str | None:
    """``NEXCLAMP_<suffix>``, falling back to the legacy ``NEURAXIS_`` and ``NEUROSEM_`` names."""
    return (os.environ.get(f"NEXCLAMP_{suffix}") or os.environ.get(f"NEURAXIS_{suffix}")
            or os.environ.get(f"NEUROSEM_{suffix}") or default)
...
def git_state(repo: Path = REPO_ROOT) -> tuple[str, bool]:
    try:
        ...
        return env_var("GIT_COMMIT", "unknown"), True
...
def environment_digest(extra: dict[str, Any] | None = None) -> tuple[str, dict[str, Any]]:
    ...
    if image := env_var("CONTAINER_IMAGE"):   # e.g. repo@sha256:..., set with docker run -e
