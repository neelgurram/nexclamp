...
checks guarantee that: every operator records an :class:`~neurosem.schemas.Edit` for each
...
from neurosem.provenance import sha256_json, tree_manifest, utc_now
from neurosem.schemas import (
    Edit,
    ModelRecord,
    MutationFamily,
    VariantKind,
...
from neurosem.units import UNITS
...
def quantity_si(text: str) -> float:
    ...
    if unit not in UNITS:
        raise MutationError(f"unit {unit!r} in {text!r} is not supported by neurosem.units")
...
def generate_mutants(model: ModelRecord, operators: Sequence[str], n_per_operator: int, seed: int,
    ...
    with tempfile.TemporaryDirectory(prefix="neurosem_mutref_") as tmp:
        ...
        for op in ops:
            for site in choose_sites(op.sites(ref), n_per_operator, seed, op.name):
