...
from neurosem.models import Workspace, copy_workspace
from neurosem.protocols import rheobase as rb
from neurosem.protocols.generate import DEFAULT_SEED, canonical_output, group_by_length, harness_step_and_length, write_probe
from neurosem.provenance import (REPO_ROOT, ImmutableWriteError, environment_digest, git_state, relpath, sha256_bytes,
                                 sha256_file, sha256_json, utc_now, write_immutable, write_immutable_text)
from neurosem.schemas import ConcreteProtocol, ExecConfig, RunRecord, RunStatus, Trace, VariantRecord, dumps
from neurosem.simulators.base import Simulator
...
class RunRecorder:
    ...
    def run_rheobase(self, ws: Workspace, variant: VariantRecord, level_exec: ExecConfig, rcfg: dict,
        ...
        with self._lock(search_id):
            ...
            digest, _ = self.env()
            commit, dirty = git_state()
            record = {"search_id": search_id, "campaign": self.campaign, "model_id": variant.model_id,
