...
(DEPENDENCY_AUDIT.md): those helpers return a bare ``False`` both for an invalid model
...
from neurosem.schemas import RunStatus, SimResult, Trace, ValidationResult
...
def find_java() -> Path | None:
    ...
    if env := os.environ.get("NEUROSEM_JAVA"):
...
def find_jar() -> Path | None:
    if env := os.environ.get("NEUROSEM_JNML_JAR"):
        p = Path(env)
...
class JNeuroML:
    ...
    def _require(self) -> tuple[Path, Path]:
        if not self.java:
            raise ToolUnavailable("Java runtime not found (set NEUROSEM_JAVA or run scripts/bootstrap_java.py)")
        if not self.jar:
    ...
    def run_lems(self, lems_file: Path, outputs: list[OutputSpec], timeout_s: float = 3600.0,
        ...
        try:
            ...
            return SimResult(RunStatus.RUNTIME_ERROR, proc.returncode, runtime, {}, cmd, f"output unreadable: {exc}", tail)
