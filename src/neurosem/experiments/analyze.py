...
def reclassify(campaign: str, multipliers: list[float]) -> pd.DataFrame:
    ...
    from neurosem.validation.fingerprint import classify, compare, load_fingerprint
...
def analyze_campaign(campaign: str, n_boot: int = 2000, n_perm: int = 2000, draws: int | None = None) -> Path:
    from neurosem.analysis import bootstrap, figures, metrics
    from neurosem.selection import greedy
