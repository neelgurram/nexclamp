...
from neurosem.models import Workspace, load_models, materialize, snapshot_dir
from neurosem.provenance import (REPO_ROOT, git_state, sha256_bytes, sha256_file, sha256_json, tree_manifest,
                                 utc_now, write_immutable_text)
from neurosem.schemas import ModelRecord, RunStatus, VariantKind, VariantRecord, dumps, to_jsonable
...
MANDATORY_EXCLUDES: tuple[str, ...] = (
    ...
    "configs/agent_policy.yaml", "configs/FROZEN.lock", "src/neurosem/experiments/agent.py", "AI_USE_LOG.md",
    "**/PROVENANCE.json",
)
...
def load_operator_registry(registry: str) -> Mapping[str, Any]:
    """Import ``neurosem.mutations`` / ``neurosem.transforms`` lazily and return its ``REGISTRY``.
...
def prepare_trial(task_id: str, dest: Path, *, private_dir: Path | None = None, private_root: Path = PRIVATE_ROOT,
    ...
    state = source_state(repo, [task.task_dir, common, policy.path, repo / "src" / "neurosem", snapshot_dir(model)])
...
class DefaultEvaluators:
    ...
    def canonical_passes(self, ctx: ScoringContext) -> LayerOutcome:
        ...
        try:
            ...
            from neurosem.validation.fingerprint import Fingerprint, compare
    ...
    def hidden_battery_passes(self, ctx: ScoringContext) -> LayerOutcome:
        ...
        if not b.get("applicable"):
            return LayerOutcome(name, LayerStatus.NOT_APPLICABLE)
        try:
            from neurosem.protocols.definitions import CANONICAL_ID, templates_from_config
            from neurosem.schemas import ExecConfig
            from neurosem.validation.canonical import canonical_protocol
            from neurosem.validation.fingerprint import RHEOBASE_ID, build_fingerprint, compare
        except ImportError as exc:
...
def score_trial(task_id: str, trial_dir: Path, frozen_config: FrozenConfig | Path | str, *,
    ...
    if not frozen_config.provisional:
        repo = Path(repo_root)
        frozen_config.check_evaluator_state(source_state(repo, [repo / "src" / "neurosem", repo / "agent_study",
                                                                repo / "configs"]))
