# NeuroSem dependency and version audit
...
- `src/neurosem/simulators/jneuroml.py`, `find_jar()`: imports `pyneuroml.utils.misc.get_path_to_jnml_jar` to find the jar. The environment variable `NEUROSEM_JNML_JAR` overrides it.
...
**How NeuroSem uses it** (all through `src/neurosem/simulators/jneuroml.py`, class `JNeuroML`):
...
- **NeuroSem code.** `src/neurosem/simulators/neuron.py` defines `NeuronSimulator`. `available()` tries `import neuron`. `run_lems` returns `RunStatus.TOOL_FAILURE` with "not implemented in the pilot". NEURON is not installed ([T2]). N-08 is deferred.
...
| T13 | `git log -1`; `git status --short`; `git diff --stat 3cdb957 HEAD`; `git diff --quiet 3cdb957 HEAD -- requirements.lock pyproject.toml`; `git show HEAD:src/neurosem/provenance.py \| grep -n ...`; `git show HEAD:src/neurosem/simulators/jneuroml.py \| grep -n ...`; `git diff DECISIONS.md` (all read-only) |
...
**Repository files:** `DECISIONS.md` (D-004 to D-023, N-01 to N-12), `docs/ARCHITECTURE.md`, `docs/REPRODUCING.md`, `docs/build_notes/{core,features,infra,mutations,transforms,selection,science-docs,model-curation}.md`, `docs/pilot/dt_probe.md`, `AI_USE_LOG.md`, `requirements.lock`, `pyproject.toml`, `Dockerfile`, `Makefile`, `environment.yml`, `.github/workflows/ci.yml`, `.tools/jdk_provenance.json`, `.tools/jdk-21.0.12.1+1/release`, `configs/features.yaml`, `configs/study.yaml`, `scripts/bootstrap_java.py`, `src/neurosem/simulators/{jneuroml,neuron}.py`, `src/neurosem/features/efel_adapter.py`, `src/neurosem/validation/structural.py`, `src/neurosem/provenance.py`, `docs/handoff/NEUROSEM_FINAL_SPEC.extracted.md`, `docs/handoff/NEUROSEM_CLAUDE_HANDOFF.extracted.md`.
