TITLE Mod file for component: Component(id=kad type=ionChannelHH)

COMMENT

    This NEURON file has been generated by org.neuroml.export (see https://github.com/NeuroML/org.neuroml.export)
         org.neuroml.export  v1.11.0
         org.neuroml.model   v1.11.0
         jLEMS               v0.12.0

ENDCOMMENT

NEURON {
    SUFFIX kad
    USEION k WRITE ik VALENCE 1 ? Assuming valence = 1; TODO check this!!
    
    RANGE gion
    RANGE i__kad : a copy of the variable for current which makes it easier to access from outside the mod file
    RANGE gmax                              : Will be changed when ion channel mechanism placed on cell!
    RANGE conductance                       : parameter
    RANGE g                                 : exposure
    RANGE fopen                             : exposure
    RANGE n_instances                       : parameter
    RANGE n_alpha                           : exposure
    RANGE n_beta                            : exposure
    RANGE n_tau                             : exposure
    RANGE n_inf                             : exposure
    RANGE n_rateScale                       : exposure
    RANGE n_fcond                           : exposure
    RANGE n_forwardRate_TIME_SCALE          : parameter
    RANGE n_forwardRate_VOLT_SCALE          : parameter
    RANGE n_forwardRate_TEMP_SCALE          : parameter
    RANGE n_forwardRate_TEMP_OFFSET         : parameter
    RANGE n_forwardRate_r                   : exposure
    RANGE n_reverseRate_TIME_SCALE          : parameter
    RANGE n_reverseRate_VOLT_SCALE          : parameter
    RANGE n_reverseRate_TEMP_SCALE          : parameter
    RANGE n_reverseRate_TEMP_OFFSET         : parameter
    RANGE n_reverseRate_r                   : exposure
    RANGE n_timeCourse_TIME_SCALE           : parameter
    RANGE n_timeCourse_VOLT_SCALE           : parameter
    RANGE n_timeCourse_t                    : exposure
    RANGE n_steadyState_TIME_SCALE          : parameter
    RANGE n_steadyState_VOLT_SCALE          : parameter
    RANGE n_steadyState_x                   : exposure
    RANGE n_q10Settings_q10Factor           : parameter
    RANGE n_q10Settings_experimentalTemp    : parameter
    RANGE n_q10Settings_TENDEGREES          : parameter
    RANGE n_q10Settings_q10                 : exposure
    RANGE l_instances                       : parameter
    RANGE l_tau                             : exposure
    RANGE l_inf                             : exposure
    RANGE l_rateScale                       : exposure
    RANGE l_fcond                           : exposure
    RANGE l_timeCourse_TIME_SCALE           : parameter
    RANGE l_timeCourse_VOLT_SCALE           : parameter
    RANGE l_timeCourse_t                    : exposure
    RANGE l_steadyState_TIME_SCALE          : parameter
    RANGE l_steadyState_VOLT_SCALE          : parameter
    RANGE l_steadyState_TEMP_SCALE          : parameter
    RANGE l_steadyState_TEMP_OFFSET         : parameter
    RANGE l_steadyState_x                   : exposure
    RANGE l_q10Settings_fixedQ10            : parameter
    RANGE l_q10Settings_q10                 : exposure
    RANGE n_forwardRate_V                   : derived variable
    RANGE n_forwardRate_celsius             : derived variable
    RANGE n_reverseRate_V                   : derived variable
    RANGE n_reverseRate_celsius             : derived variable
    RANGE n_timeCourse_V                    : derived variable
    RANGE n_timeCourse_ALPHA                : derived variable
    RANGE n_timeCourse_BETA                 : derived variable
    RANGE n_steadyState_V                   : derived variable
    RANGE n_steadyState_ALPHA               : derived variable
    RANGE n_steadyState_BETA                : derived variable
    RANGE n_tauUnscaled                     : derived variable
    RANGE l_timeCourse_V                    : derived variable
    RANGE l_steadyState_V                   : derived variable
    RANGE l_steadyState_celsius             : derived variable
    RANGE l_tauUnscaled                     : derived variable
    RANGE conductanceScale                  : derived variable
    RANGE fopen0                            : derived variable
    
}

UNITS {
    
    (nA) = (nanoamp)
    (uA) = (microamp)
    (mA) = (milliamp)
    (A) = (amp)
    (mV) = (millivolt)
    (mS) = (millisiemens)
    (uS) = (microsiemens)
    (nF) = (nanofarad)
    (molar) = (1/liter)
    (kHz) = (kilohertz)
    (mM) = (millimolar)
    (um) = (micrometer)
    (umol) = (micromole)
    (pC) = (picocoulomb)
    (S) = (siemens)
    
}

PARAMETER {
    
    gmax = 0  (S/cm2)                       : Will be changed when ion channel mechanism placed on cell!
    
    conductance = 1.0E-5 (uS)              : was: 1.0E-11 (conductance)
    n_instances = 1                        : was: 1.0 (none)
    n_forwardRate_TIME_SCALE = 1 (ms)      : was: 0.001 (time)
    n_forwardRate_VOLT_SCALE = 1 (mV)      : was: 0.001 (voltage)
    n_forwardRate_TEMP_SCALE = 1 (K)       : was: 1.0 (temperature)
    n_forwardRate_TEMP_OFFSET = 273.15 (K) : was: 273.15 (temperature)
    n_reverseRate_TIME_SCALE = 1 (ms)      : was: 0.001 (time)
    n_reverseRate_VOLT_SCALE = 1 (mV)      : was: 0.001 (voltage)
    n_reverseRate_TEMP_SCALE = 1 (K)       : was: 1.0 (temperature)
    n_reverseRate_TEMP_OFFSET = 273.15 (K) : was: 273.15 (temperature)
    n_timeCourse_TIME_SCALE = 1 (ms)       : was: 0.001 (time)
    n_timeCourse_VOLT_SCALE = 1 (mV)       : was: 0.001 (voltage)
    n_steadyState_TIME_SCALE = 1 (ms)      : was: 0.001 (time)
    n_steadyState_VOLT_SCALE = 1 (mV)      : was: 0.001 (voltage)
    n_q10Settings_q10Factor = 5            : was: 5.0 (none)
    n_q10Settings_experimentalTemp = 297.15 (K): was: 297.15 (temperature)
    n_q10Settings_TENDEGREES = 10 (K)      : was: 10.0 (temperature)
    l_instances = 1                        : was: 1.0 (none)
    l_timeCourse_TIME_SCALE = 1 (ms)       : was: 0.001 (time)
    l_timeCourse_VOLT_SCALE = 1 (mV)       : was: 0.001 (voltage)
    l_steadyState_TIME_SCALE = 1 (ms)      : was: 0.001 (time)
    l_steadyState_VOLT_SCALE = 1 (mV)      : was: 0.001 (voltage)
    l_steadyState_TEMP_SCALE = 1 (K)       : was: 1.0 (temperature)
    l_steadyState_TEMP_OFFSET = 273.15 (K) : was: 273.15 (temperature)
    l_q10Settings_fixedQ10 = 1             : was: 1.0 (none)
}

ASSIGNED {
    
    gion   (S/cm2)                          : Transient conductance density of the channel? Standard Assigned variables with ionChannel
    v (mV)
    celsius (degC)
    temperature (K)
    ek (mV)
    ik (mA/cm2)
    i__kad (mA/cm2)
    
    n_forwardRate_V                         : derived variable
    n_forwardRate_celsius                   : derived variable
    n_forwardRate_r (kHz)                   : derived variable
    n_reverseRate_V                         : derived variable
    n_reverseRate_celsius                   : derived variable
    n_reverseRate_r (kHz)                   : derived variable
    n_timeCourse_V                          : derived variable
    n_timeCourse_ALPHA                      : derived variable
    n_timeCourse_BETA                       : derived variable
    
    n_timeCourse_t (ms)                     : conditional derived var...
    n_steadyState_V                         : derived variable
    n_steadyState_ALPHA                     : derived variable
    n_steadyState_BETA                      : derived variable
    n_steadyState_x                         : derived variable
    n_q10Settings_q10                       : derived variable
    n_rateScale                             : derived variable
    n_alpha (kHz)                           : derived variable
    n_beta (kHz)                            : derived variable
    n_inf                                   : derived variable
    n_tauUnscaled (ms)                      : derived variable
    n_tau (ms)                              : derived variable
    n_fcond                                 : derived variable
    l_timeCourse_V                          : derived variable
    
    l_timeCourse_t (ms)                     : conditional derived var...
    l_steadyState_V                         : derived variable
    l_steadyState_celsius                   : derived variable
    l_steadyState_x                         : derived variable
    l_q10Settings_q10                       : derived variable
    l_rateScale                             : derived variable
    l_fcond                                 : derived variable
    l_inf                                   : derived variable
    l_tauUnscaled (ms)                      : derived variable
    l_tau (ms)                              : derived variable
    conductanceScale                        : derived variable
    fopen0                                  : derived variable
    fopen                                   : derived variable
    g (uS)                                  : derived variable
    rate_n_q (/ms)
    rate_l_q (/ms)
    
}

STATE {
    n_q  : dimension: none
    l_q  : dimension: none
    
}

INITIAL {
    ek = -90.0
    
    temperature = celsius + 273.15
    
    rates()
    rates() ? To ensure correct initialisation.
    
    n_q = n_inf
    
    l_q = l_inf
    
}

BREAKPOINT {
    
    SOLVE states METHOD cnexp
    
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=kad type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=kad type=ionChannelHH), from gates; Component(id=n type=gateHHratesTauInf)
    ? multiply applied to all instances of fcond in: <gates> ([Component(id=n type=gateHHratesTauInf), Component(id=l type=gateHHtauInf)]))
    fopen0 = n_fcond * l_fcond ? path based, prefix = 
    
    fopen = conductanceScale  *  fopen0 ? evaluable
    g = conductance  *  fopen ? evaluable
    gion = gmax * fopen 
    
    ik = gion * (v - ek)
    i__kad =  -1 * ik : set this variable to the current also - note -1 as channel current convention for LEMS used!
    
}

DERIVATIVE states {
    rates()
    n_q' = rate_n_q 
    l_q' = rate_l_q 
    
}

PROCEDURE rates() {
    
    n_forwardRate_V = v /  n_forwardRate_VOLT_SCALE ? evaluable
    n_forwardRate_celsius = (temperature -  n_forwardRate_TEMP_OFFSET ) /  n_forwardRate_TEMP_SCALE ? evaluable
    n_forwardRate_r = ((exp ( (1e-3 * (-1.8 + (-1)/(1 + (exp (( n_forwardRate_V -(-40))/5)))) * ( n_forwardRate_V  - (-1)) * 9.648e4) / (8.315*(273.16 + ( n_forwardRate_celsius ) )) ))) /  n_forwardRate_TIME_SCALE ? evaluable
    n_reverseRate_V = v /  n_reverseRate_VOLT_SCALE ? evaluable
    n_reverseRate_celsius = (temperature -  n_reverseRate_TEMP_OFFSET ) /  n_reverseRate_TEMP_SCALE ? evaluable
    n_reverseRate_r = ((exp ( (1e-3 * (-1.8 + (-1)/(1 + (exp (( n_reverseRate_V -(-40))/5)))) * 0.39 * ( n_reverseRate_V  - (-1)) * 9.648e4) / (8.315*(273.16 + ( n_reverseRate_celsius ) )) ))) /  n_reverseRate_TIME_SCALE ? evaluable
    n_timeCourse_V = v /  n_timeCourse_VOLT_SCALE ? evaluable
    n_timeCourse_ALPHA = n_alpha  *  n_timeCourse_TIME_SCALE ? evaluable
    n_timeCourse_BETA = n_beta  *  n_timeCourse_TIME_SCALE ? evaluable
    if (( n_timeCourse_ALPHA  +  n_timeCourse_BETA ) == 0)  { 
        n_timeCourse_t = 0.0  *  n_timeCourse_TIME_SCALE ? evaluable cdv
    } else if (n_timeCourse_BETA /(0.1 * (1 +  n_timeCourse_ALPHA ) *  n_rateScale )  < ( 0.2 ))  { 
        n_timeCourse_t = ( (0.2 *  n_rateScale ) ) *  n_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
        n_timeCourse_t = (  n_timeCourse_BETA /(0.1 * (1 +  n_timeCourse_ALPHA )) ) *  n_timeCourse_TIME_SCALE ? evaluable cdv
    }
    
    n_steadyState_V = v /  n_steadyState_VOLT_SCALE ? evaluable
    n_steadyState_ALPHA = n_alpha  *  n_steadyState_TIME_SCALE ? evaluable
    n_steadyState_BETA = n_beta  *  n_steadyState_TIME_SCALE ? evaluable
    n_steadyState_x = 1/(1 +  n_steadyState_ALPHA ) ? evaluable
    n_q10Settings_q10 = n_q10Settings_q10Factor ^((temperature -  n_q10Settings_experimentalTemp )/ n_q10Settings_TENDEGREES ) ? evaluable
    ? DerivedVariable is based on path: q10Settings[*]/q10, on: Component(id=n type=gateHHratesTauInf), from q10Settings; Component(id=null type=q10ExpTemp)
    ? multiply applied to all instances of q10 in: <q10Settings> ([Component(id=null type=q10ExpTemp)]))
    n_rateScale = n_q10Settings_q10 ? path based, prefix = n_
    
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=n type=gateHHratesTauInf), from forwardRate; Component(id=null type=kad_n_alpha_rate)
    n_alpha = n_forwardRate_r ? path based, prefix = n_
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=n type=gateHHratesTauInf), from reverseRate; Component(id=null type=kad_n_beta_rate)
    n_beta = n_reverseRate_r ? path based, prefix = n_
    
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=n type=gateHHratesTauInf), from steadyState; Component(id=null type=kad_n_inf_inf)
    n_inf = n_steadyState_x ? path based, prefix = n_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=n type=gateHHratesTauInf), from timeCourse; Component(id=null type=kad_n_tau_tau)
    n_tauUnscaled = n_timeCourse_t ? path based, prefix = n_
    
    n_tau = n_tauUnscaled  /  n_rateScale ? evaluable
    n_fcond = n_q ^ n_instances ? evaluable
    l_timeCourse_V = v /  l_timeCourse_VOLT_SCALE ? evaluable
    if (0.26*( l_timeCourse_V  + 50)  < ( 2 ))  { 
        l_timeCourse_t = 2.0  *  l_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
        l_timeCourse_t = ( 0.26*( l_timeCourse_V  + 50)) *  l_timeCourse_TIME_SCALE ? evaluable cdv
    }
    
    l_steadyState_V = v /  l_steadyState_VOLT_SCALE ? evaluable
    l_steadyState_celsius = (temperature -  l_steadyState_TEMP_OFFSET ) /  l_steadyState_TEMP_SCALE ? evaluable
    l_steadyState_x = 1/(1 + (exp ( (1e-3 * (3) * ( l_steadyState_V  - (-56)) * 9.648e4) / (8.315*(273.16 + ( l_steadyState_celsius ) )) ))) ? evaluable
    l_q10Settings_q10 = l_q10Settings_fixedQ10 ? evaluable
    ? DerivedVariable is based on path: q10Settings[*]/q10, on: Component(id=l type=gateHHtauInf), from q10Settings; Component(id=null type=q10Fixed)
    ? multiply applied to all instances of q10 in: <q10Settings> ([Component(id=null type=q10Fixed)]))
    l_rateScale = l_q10Settings_q10 ? path based, prefix = l_
    
    l_fcond = l_q ^ l_instances ? evaluable
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=l type=gateHHtauInf), from steadyState; Component(id=null type=kad_l_inf_inf)
    l_inf = l_steadyState_x ? path based, prefix = l_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=l type=gateHHtauInf), from timeCourse; Component(id=null type=kad_l_tau_tau)
    l_tauUnscaled = l_timeCourse_t ? path based, prefix = l_
    
    l_tau = l_tauUnscaled  /  l_rateScale ? evaluable
    
     
    
     
    
     
    
     
    
     
    
     
    
     
    
     
    
     
    
     
    rate_n_q = ( n_inf  -  n_q ) /  n_tau ? Note units of all quantities used here need to be consistent!
    
     
    
     
    
     
    
     
    
     
    
     
    rate_l_q = ( l_inf  -  l_q ) /  l_tau ? Note units of all quantities used here need to be consistent!
    
     
    
     
    
     
    
     
    
}

