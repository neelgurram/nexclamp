...
PARAMETER {
    ...
    m_steadyState_midpoint = -56 (mV)      : was: -0.056 (voltage)
    m_steadyState_scale = 6.2 (mV)         : was: 0.006200000000000001 (voltage)
...
BREAKPOINT {
    ...
    icat = gion * (v - ecat)
    i__cat =  -1 * icat : set this variable to the current also - note -1 as channel current convention for LEMS used!
