...
NEURON {
    ...
    RANGE gion
    RANGE i__nap : a copy of the variable for current which makes it easier to access from outside the mod file
...
PARAMETER {
    ...
    m_steadyState_midpoint = -48 (mV)      : was: -0.048 (voltage)
    m_steadyState_scale = 10 (mV)          : was: 0.01 (voltage)
}
...
BREAKPOINT {
    ...
    ina = gion * (v - ena)
    i__nap =  -1 * ina : set this variable to the current also - note -1 as channel current convention for LEMS used!
