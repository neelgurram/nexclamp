...
NEURON {
    ...
    RANGE gion
    RANGE i__kv : a copy of the variable for current which makes it easier to access from outside the mod file
...
PARAMETER {
    ...
    n_forwardRate_rate = 0.18 (kHz)        : was: 180.0 (per_time)
    n_forwardRate_midpoint = 25 (mV)       : was: 0.025 (voltage)
    n_forwardRate_scale = 9 (mV)           : was: 0.009000000000000001 (voltage)
    n_reverseRate_rate = 0.018000001 (kHz) : was: 18.0 (per_time)
    n_reverseRate_midpoint = 25 (mV)       : was: 0.025 (voltage)
...
BREAKPOINT {
    ...
    ik = gion * (v - ek)
    i__kv =  -1 * ik : set this variable to the current also - note -1 as channel current convention for LEMS used!
