...
NEURON {
    ...
    RANGE gion
    RANGE i__kca : a copy of the variable for current which makes it easier to access from outside the mod file
...
PARAMETER {
    ...
    n_forwardRate_CONC_SCALE = 1 (mM)      : was: 1.0 (concentration)
    n_reverseRate_TIME_SCALE = 1 (ms)      : was: 0.001 (time)
    n_reverseRate_VOLT_SCALE = 1 (mV)      : was: 0.001 (voltage)
    n_reverseRate_CONC_SCALE = 1 (mM)      : was: 1.0 (concentration)
...
BREAKPOINT {
    ...
    ik = gion * (v - ek)
    i__kca =  -1 * ik : set this variable to the current also - note -1 as channel current convention for LEMS used!
