...
NEURON {
    ...
    RANGE gion
    RANGE i__kChan : a copy of the variable for current which makes it easier to access from outside the mod file
...
PARAMETER {
    ...
    n_forwardRate_rate = 0.1 (kHz)         : was: 100.0 (per_time)
    n_forwardRate_midpoint = -55 (mV)      : was: -0.055 (voltage)
    n_forwardRate_scale = 10 (mV)          : was: 0.01 (voltage)
    n_reverseRate_rate = 0.125 (kHz)       : was: 125.0 (per_time)
    n_reverseRate_midpoint = -65 (mV)      : was: -0.065 (voltage)
    n_reverseRate_scale = -80 (mV)         : was: -0.08 (voltage)
}
...
BREAKPOINT {
    ...
    ik = gion * (v - ek)
    i__kChan =  -1 * ik : set this variable to the current also - note -1 as channel current convention for LEMS used!
